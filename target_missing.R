...
xi2_R_arm1 <- c(0.6, 0.4, 0.4, 0.4)
xi2_R_arm0 <- c(0.6, 0.4, 0.4, 0.4)

xi3_R_arm1 <- c(0.6, 0.5, 0.5, 0.5)
xi3_R_arm0 <- c(0.6, 0.5, 0.5, 0.5)

tgt_R_arm1 <- c(0.6, 0.4, 0.4, 0.4, 0.4)
tgt_R_arm0 <- c(0.6, 0.4, 0.4, 0.4, 0.4)
...
X_tgt_A1 <- as.matrix(Target[which(Target$trt==1), c("V2", "V3", "V4", "V5")])
X_tgt_A0 <- as.matrix(Target[which(Target$trt==0), c("V2", "V3", "V4", "V5")])
X_tgt_A1 <- cbind(rep(1, nrow(X_tgt_A1)), X_tgt_A1)
X_tgt_A0 <- cbind(rep(1, nrow(X_tgt_A0)), X_tgt_A0)
Target_R1 <- X_tgt_A1 %*% tgt_R_arm1
Target_R0 <- X_tgt_A0 %*% tgt_R_arm0
Target[which(Target$trt==1), "R"] <- Y_gen(Target_R1)
Target[which(Target$trt==0), "R"] <- Y_gen(Target_R0)
Target[which(Target$R==0), "V1"] <- NA
...
for(j in 1:length(TrialMiss)){
  if(j==1){
    ...
    X_trl_A1 <- as.matrix(TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),c("V3", "V4", "V5")])
    X_trl_A0 <- as.matrix(TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),c("V3", "V4", "V5")])
    X_trl_A1 <- cbind(rep(1, nrow(X_trl_A1)), X_trl_A1)
    X_trl_A0 <- cbind(rep(1, nrow(X_trl_A0)), X_trl_A0)
    Trial_V1_R1 <- Y_gen(X_trl_A1 %*% xi2_R_arm1)
    Trial_V1_R0 <- Y_gen(X_trl_A0 %*% xi2_R_arm0)  
    Trial_V2_R1 <- Y_gen(X_trl_A1 %*% xi2_R_arm1)
    Trial_V2_R0 <- Y_gen(X_trl_A0 %*% xi2_R_arm0)  
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"R"] <- apply(cbind(Trial_V1_R1, Trial_V2_R1), 1, function(x){all(x==1)})
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"R"] <- apply(cbind(Trial_V1_R0, Trial_V2_R0), 1, function(x){all(x==1)})
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"V1"][Trial_V1_R1==0] <- NA
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"V1"][Trial_V1_R0==0] <- NA
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"V2"][Trial_V2_R1==0] <- NA
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"V2"][Trial_V2_R0==0] <- NA
  }else if(j==3){
    X_trl_A1 <- as.matrix(TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),c("V2", "V4", "V5")])
    X_trl_A0 <- as.matrix(TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),c("V2", "V4", "V5")])
    X_trl_A1 <- cbind(rep(1, nrow(X_trl_A1)), X_trl_A1)
    X_trl_A0 <- cbind(rep(1, nrow(X_trl_A0)), X_trl_A0)
    Trial_V1_R1 <- Y_gen(X_trl_A1 %*% xi2_R_arm1)
    Trial_V1_R0 <- Y_gen(X_trl_A0 %*% xi2_R_arm0)  
    Trial_V3_R1 <- Y_gen(X_trl_A1 %*% xi2_R_arm1)
    Trial_V3_R0 <- Y_gen(X_trl_A0 %*% xi2_R_arm0)  
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"R"] <- apply(cbind(Trial_V1_R1, Trial_V3_R1), 1, function(x){all(x==1)})
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"R"] <- apply(cbind(Trial_V1_R0, Trial_V3_R0), 1, function(x){all(x==1)})
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"V1"][Trial_V1_R1==0] <- NA
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"V1"][Trial_V1_R0==0] <- NA
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"V3"][Trial_V3_R1==0] <- NA
    TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"V3"][Trial_V3_R0==0] <- NA
  }
}
...
Phi_hat <- function(a, Trials){
  ...
  Whole <- rbind(Trials[, c("V1", "V2", "V3", "V4", "V5", "trt", "Y", "R")], Target[Target$R==1, ])
  In_Trial <- c(rep(1, nrow(Trials)), rep(0, nrow(Target[Target$R==1, ])))
  Whole <- cbind(Whole, In_Trial)
  Trial_whole <- rbind(Trial[, c("V1", "V2", "V3", "V4", "V5", "trt", "Y", "R")], Target[Target$R==1, ])
  #### compute hat_p
  ...
  ### compute target pr
  tgt_pr_fit <- glm(R~V2+V3+V4+V5, data=Target, family="binomial")
  tgt_pr <- sapply(predict(tgt_pr_fit, Whole), expit) 
  ...
  if(a==1){
   ...
   ### outcome estimator
  out <- sum((ga/tgt_pr)*(In_Trial==0))/nrow(Target)

  ### IPW estimator
  #IPW <- sum((In_Trial==1)*(Whole$trt==a)*wa*Whole$Y/tgt_pr)/nrow(Target)
  #IPW_c <- sum((In_Trial==1)*(Whole$trt==a)*wa_c*Whole$Y/tgt_pr)/nrow(Target)

  ### normalized DR estimator
  #DR <- out + sum((In_Trial==1)*(Whole$trt==a)*wa*(Whole$Y-ga)/tgt_pr)/nrow(Target)
  #DR_c <- out + sum((In_Trial==1)*(Whole$trt==a)*wa_c*(Whole$Y-ga)/tgt_pr)/nrow(Target)
...
Phi_mi <- function(a, Trials){
  ...
  covariates <-  paste(variable_name, collapse="+")
  covariates_outcome <- paste0(covariates, "+I(V2^2)")
  incomplete_data <- Target[, c("V1", "V2", "V3", "V4", "V5")]
  imp <- mice(incomplete_data, m=5, maxit = 50, method = 'pmm', seed = 500, print=F)
  complete_data <- complete(imp, 2)
  Target_cpt <- cbind(complete_data[,c("V1", "V2", "V3", "V4", "V5")], Target[,c("trt", "Y", "R")])
  Whole <- rbind(Trials[, c("V1", "V2", "V3", "V4", "V5", "trt", "Y", "R")], Target_cpt)
  In_Trial <- c(rep(1, nrow(Trials)), rep(0, nrow(Target_cpt)))
  Whole <- cbind(Whole, In_Trial)
  ...
  if(a==1){
   ...
   ### outcome estimator
  out <- sum(ga*(In_Trial==0))/nrow(Target_cpt)

  ### IPW estimator
  IPW <- sum((In_Trial==1)*(Whole$trt==a)*wa*Whole$Y)/nrow(Target_cpt)

  ### normalized DR estimator
  DR <- out + sum((In_Trial==1)*(Whole$trt==a)*wa*(Whole$Y-ga))/nrow(Target_cpt)
...
write.csv(Res, "target_missing.csv")
