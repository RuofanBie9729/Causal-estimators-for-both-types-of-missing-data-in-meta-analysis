...
Phi_hat <- function(a, Trials){
  ...
  hat_p_crt_fit <- glm(R~V2+V3+V4, data=Trial_whole, family="binomial") 
  ...
  ea0 <- 1-ea1
  ea_crt_fit <- glm(R~V2+V3+V4, data=Trial, family="binomial")
  ea_crt <- sapply(predict(ea_crt_fit, newdata=Whole), expit) 
  ### compute pr

  Trial_arm <- Trial[which(Trial$trt==a),]
  pr_fit <- glm(R~V2+V3+V4, data=Trial_arm, family="binomial")
