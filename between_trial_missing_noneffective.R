...
beta_vec <- c(0.7, -0.7, 0, 0.7, -0.7, 0.2) 

###coefficients for outcome generating model
theta1 <- c(1, 0.5, 0.5, 0, 0.5, 0.5, 0.1) 
theta0 <- c(1, -0.5, -0.5, 0, -0.5, -0.5, -0.1)
...
xi1_R_arm1 <- c(0.6, 0.3, 0.3, 0.3)
xi1_R_arm0 <- c(0.6, 0.3, 0.3, 0.3)
...
Phi_hat <- function(a, Trials){
  ...
  variable_name <- variable_name[!apply(Trials[,1:p], 2, function(x){all(is.na(x))})]
...
ak_c <- sapply(TrialMiss, nrow, simplify=T)[2:3]/sum(sapply(TrialMiss, nrow, simplify=T)[2:3])
...
Phi_mi <- function(a, Trials){
  ...
  #variable_name <- variable_name[!is.na(apply(Trials[,1:p], 2, mean))] ### used for complete-variable estimators 
                                                                        ### when between trial missingness exists
  #variable_name <- c("V1", variable_name)
  covariates <-  paste(variable_name, collapse="+")
  covariates_outcome <- paste0(covariates, "+I(V2^2)")
...
merge_data <- do.call("rbind", TrialMiss)
incomplete_data <- merge_data[, c("V1", "V2", "V3", "V4", "V5")]
...
Trials_list <- lapply(complete_data_list, function(x){
     cbind(x[,c("V1", "V2", "V3", "V4", "V5")],
        merge_data[,c("trial", "trt", "Y", "R")])})
mi_res1 <- apply(sapply(Trials_list, Phi_mi, a=1, simplify=T), 1, mean)
mi_res0 <- apply(sapply(Trials_list, Phi_mi, a=0, simplify=T), 1, mean)

Res <- rbind(Res, c(pool(1)-pool(0), EstRes1[1:3,]%*%ak-EstRes0[1:3,]%*%ak, 
   EstRes1[4:5,2:3]%*%ak_c-EstRes0[4:5,2:3]%*%ak_c,
   mi_res1-mi_res0, Phi(1)-Phi(0)))
if(i%%100==0){
  print(i)
}
}
...
a <- apply(Res, 2, mean)
 a[1:9]-a[10]
write.csv(Res, "between_trial_missing(non-effective).csv")
