...
xi_R_arm1 <- c(0.5, 0.1, 0.1, 0.2)
xi_R_arm0 <- c(0.5, 0.1, 0.1, 0.2)
...
for(j in 1:length(TrialMiss)){
  ...
  Trial_R1 <- X_trl_A1 %*% xi_R_arm1
  Trial_R0 <- X_trl_A0 %*% xi_R_arm0
  TrialMiss[[j]][which(TrialMiss[[j]]$trt==1),"R"] <- Y_gen(Trial_R1)
  TrialMiss[[j]][which(TrialMiss[[j]]$trt==0),"R"] <- Y_gen(Trial_R0)
  #TrialMiss[[j]]$trueV1 <- TrialMiss[[i]]$V1
  #TrialMiss[[j]][which(TrialMiss[[i]]$R==0),"V1"] <- NA
}
...
Phi_hat <- function(a, Trials){
  ...
  hat_p_crt_fit <- glm(R~V2, data=Trial_whole, family="binomial") 
  ...
  ea0 <- 1-ea1
  ea_crt_fit <- glm(R~V2, data=Trial, family="binomial")
  ea_crt <- sapply(predict(ea_crt_fit, newdata=Whole), expit) 
  ### compute pr

  Trial_arm <- Trial[which(Trial$trt==a),]
  pr_fit <- glm(R~V2, data=Trial_arm, family="binomial")
...
a <- apply(Res, 2, mean)
 a[1:6]-a[7]
write.csv(Res, "missing_model_misspecified.csv")
