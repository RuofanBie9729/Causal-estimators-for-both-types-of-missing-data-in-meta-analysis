...
beta_vec <- c(0.7, -0.7, 0.7, -0.7, 0.2) 

###coefficients for outcome generating model
theta1 <- c(1, 0, 0.5, 0, 0.5, 0.1) 
theta0 <- c(1, 0, -0.5, 0, -0.5, -0.1)

###coefficients for trial assignment
tau_coef <- c(log(1.3), log(1.3), log(1.3), log(1.3))
tau_coef <- rbind(tau_coef, c(log(0.8), log(0.8), log(0.8), log(0.8)))
tau_coef <- rbind(tau_coef, rep(0, 4)) 

###coefficients for missing data R=0
xi_R <- c(0.5, 0.1, 0.1, 0.2)
...
Res <- NULL
for(i in 1: 1000){
Subjects <- rmvnorm(N, rep(0, p/2), Sigma)
Subjects <- cbind(Subjects, matrix(rbinom(N*(p/2), 1, 0.5), ncol=(p/2)))
...
colnames(TrialData) <- c("V1", "V2", "V3", "V4", "trial", "trt", "Y")

Target <- as.data.frame(Target)
colnames(Target) <- c("V1", "V2", "V3", "V4", "trt", "Y")
...
Phi_hat <- function(a, Trials){
  ...
  variable_name <- variable_name[!is.na(apply(Trials[,1:p], 2, mean))]
  covariates <-  paste(variable_name, collapse="+")
  covariates_outcome <- paste0(covariates, "+I(V2^2)")
  X_trl <- as.matrix(cbind(rep(1, dim(Trials)[1]), Trials[, c("V2", "V3", "V4")]))
  X_trl_A1 <- X_trl[which(Trials$trt==1), ]
  X_trl_A0 <- X_trl[which(Trials$trt==0), ]
  Trial_R1 <- X_trl_A1 %*% xi_R
  Trial_R0 <- X_trl_A0 %*% xi_R
  Trials[which(Trials$trt==1), "R"] <- Y_gen(Trial_R1)
  Trials[which(Trials$trt==0), "R"] <- Y_gen(Trial_R0)
  Trial <- Trials
  Trials <- Trials[Trials$R==1,]
  Target$R <- rep(1, nrow(Target))
  Whole <- rbind(Trials[, c("V1", "V2", "V3", "V4", "trt", "Y", "R")], Target)
  In_Trial <- c(rep(1, nrow(Trials)), rep(0, nrow(Target)))
  Whole <- cbind(Whole, In_Trial)
  Trial_whole <- rbind(Trial[, c("V1", "V2", "V3", "V4", "trt", "Y", "R")], Target)
  #### compute hat_p
  ...
  ### IPW estimator
  IPW <- sum((In_Trial==1)*(Whole$trt==a)*(Whole$R==1)*wa*Whole$Y)/nrow(Target)
  IPW_c <- sum((In_Trial==1)*(Whole$trt==a)*(Whole$R==1)*wa_c*Whole$Y)/nrow(Target)

  ### normalized DR estimator
  DR <- out + sum((In_Trial==1)*(Whole$trt==a)*(Whole$R==1)*wa*(Whole$Y-ga))/nrow(Target)
  DR_c <- out + sum((In_Trial==1)*(Whole$trt==a)*(Whole$R==1)*wa_c*(Whole$Y-ga))/nrow(Target)
...
pool <- function(a){
  if(a==1){
    poolest <- mean(Y_gen(Trial_Y1))
  }else{
    poolest <- mean(Y_gen(Trial_Y0))
  }
  return(poolest)
}
...
Res <- rbind(Res, c(pool(1)-pool(0), EstRes1%*%ak-EstRes0%*%ak, Phi(1)-Phi(0)))
if(i%%100==0){
  print(i)
}
}
